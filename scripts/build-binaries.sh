#!/usr/bin/env bash
#
# Build dg-pi binaries for all platforms locally.
...
#     dg-pi-darwin-arm64.tar.gz
#     dg-pi-darwin-x64.tar.gz
#     dg-pi-linux-x64.tar.gz
#     dg-pi-linux-arm64.tar.gz
#     dg-pi-windows-x64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == "windows-x64" ]]; then
        bun build --compile --external koffi --target=bun-$platform ./dist/bun/cli.js --outfile binaries/$platform/dg-pi.exe
    else
        bun build --compile --external koffi --target=bun-$platform ./dist/bun/cli.js --outfile binaries/$platform/dg-pi
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == "windows-x64" ]]; then
        # Windows (zip)
        echo "Creating dg-pi-$platform.zip..."
        (cd $platform && zip -r ../dg-pi-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating dg-pi-$platform.tar.gz..."
        mv $platform dg-pi && tar -czf dg-pi-$platform.tar.gz dg-pi && mv dg-pi $platform
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf $platform
    if [[ "$platform" == "windows-x64" ]]; then
        mkdir -p $platform && (cd $platform && unzip -q ../dg-pi-$platform.zip)
    else
        tar -xzf dg-pi-$platform.tar.gz && mv dg-pi $platform
    fi
done
