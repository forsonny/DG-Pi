...
- Fixed `continue()` to resume queued steering/follow-up messages when context currently ends in an assistant message, and preserved one-at-a-time steering ordering during assistant-tail resumes ([#1312](https://github.com/forsonny/DG-Pi/pull/1312) by [@ferologics](https://github.com/ferologics))
...
- Added `maxRetryDelayMs` option to `AgentOptions` to cap server-requested retry delays. Passed through to the underlying stream function. ([#1123](https://github.com/forsonny/DG-Pi/issues/1123))
...
- `thinkingBudgets` option on `Agent` and `AgentOptions` to customize token budgets per thinking level ([#529](https://github.com/forsonny/DG-Pi/pull/529) by [@melihmucuk](https://github.com/melihmucuk))
...
- **Agent loop moved from `@dg-forsonny/dg-pi-ai`**: The `agentLoop`, `agentLoopContinue`, and related types have moved to this package. Import from `@dg-forsonny/dg-pi-agent-core` instead.
