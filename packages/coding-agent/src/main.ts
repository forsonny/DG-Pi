...
import { type ImageContent, modelsAreEqual, supportsXhigh } from "@dg-forsonny/dg-pi-ai";
...
export async function main(args: string[]) {
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.DG_PI_OFFLINE);
	if (offlineMode) {
		process.env.DG_PI_OFFLINE = "1";
		process.env.DG_PI_SKIP_VERSION_CHECK = "1";
	}
	...
	const resourceLoader = new DefaultResourceLoader({
		...
		additionalAgentPaths: firstPass.agents,
		additionalPromptTemplatePaths: firstPass.promptTemplates,
		additionalThemePaths: firstPass.themes,
		noExtensions: firstPass.noExtensions,
		noSkills: firstPass.noSkills,
		noAgents: firstPass.noAgents,
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.DG_PI_STARTUP_BENCHMARK);
	if (startupBenchmark && !isInteractive) {
		console.error(chalk.red("Error: DG_PI_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
