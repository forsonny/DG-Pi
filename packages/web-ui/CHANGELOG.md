...
- Exported `CustomProviderDialog` from `@dg-forsonny/dg-pi-web-ui` ([#2267](https://github.com/forsonny/DG-Pi/issues/2267))
...
- Build `@dg-forsonny/dg-pi-web-ui` with `tsc` instead of `tsgo` so Lit decorator-based state updates rerender correctly.
...
- Made model selector search case-insensitive by normalizing query tokens, fixing auto-capitalized mobile input filtering ([#1443](https://github.com/forsonny/DG-Pi/issues/1443))
...
- Exported `CustomProviderCard`, `ProviderKeyInput`, `AbortedMessage`, and `ToolMessageDebugView` components for custom UIs ([#1015](https://github.com/forsonny/DG-Pi/issues/1015))
...
- Updated tsgo to 7.0.0-dev.20260120.1 for decorator support ([#873](https://github.com/forsonny/DG-Pi/issues/873))
...
- **Agent class moved to `@dg-forsonny/dg-pi-agent-core`**: The `Agent` class, `AgentState`, and related types are no longer exported from this package. Import them from `@dg-forsonny/dg-pi-agent-core` instead.
...
- **`AppMessage` renamed to `AgentMessage`**: Now imported from `@dg-forsonny/dg-pi-agent-core`. Custom message types use declaration merging on `CustomAgentMessages` interface.
...
- **`CustomMessages` interface removed**: Use declaration merging on `CustomAgentMessages` from `@dg-forsonny/dg-pi-agent-core` instead.

- **`agent.appendMessage()` removed**: Use `agent.queueMessage()` instead.

- **Agent event types changed**: `AgentInterface` now handles new event types from `@dg-forsonny/dg-pi-agent-core`: `message_start`, `message_end`, `message_update`, `turn_start`, `turn_end`, `agent_start`, `agent_end`.
...
- `Agent` class (moved to `@dg-forsonny/dg-pi-agent-core`)
...
**Before (0.30.x):**
```typescript
import { Agent, ProviderTransport, type AppMessage } from '@dg-forsonny/dg-pi-web-ui';
...
**After:**
```typescript
import { Agent, type AgentMessage } from '@dg-forsonny/dg-pi-agent-core';
import { defaultConvertToLlm } from '@dg-forsonny/dg-pi-web-ui';
...
declare module "@dg-forsonny/dg-pi-web-ui" {
...
declare module "@dg-forsonny/dg-pi-agent-core" {
