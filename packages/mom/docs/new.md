...
```typescript
import { type AgentSessionEvent } from "@dg-forsonny/dg-pi-coding-agent";
...
import type { MomCustomTool, ToolAPI } from "@dg-forsonny/dg-pi-mom";
...
2. `~/.dg-pi/mom/tools/**/index.ts` (global, recursive)
...
Tools are loaded via jiti. They can import any 3rd party dependencies (install in the tool directory). Imports of `@dg-forsonny/dg-pi-ai` and `@dg-forsonny/dg-pi-mom` are aliased to the running mom bundle.
