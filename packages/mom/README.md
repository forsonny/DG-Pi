...
```bash
npm install @dg-forsonny/dg-pi-mom
```
...
# Option 2: use /login command in pi agent, then copy/link auth.json to ~/.dg-pi/mom/
...
- run interactive coding agent session: `npx @dg-forsonny/dg-pi-coding-agent`
- enter `/login` command
  - choose "Anthropic" provider
  - follow instructions in the browser
- link `auth.json` to mom: `ln -s ~/.dg-pi/agent/auth.json ~/.dg-pi/mom/auth.json`
...
Update mom anytime with `npm install -g @dg-forsonny/dg-pi-mom`. This only updates the Node.js app on your host. Anything mom installed inside the Docker container remains unchanged.
