...
chmod +x /tmp/model_run_${name}.sh`,
	...
	const env = [
		...
		`PI_API_KEY='${process.env.DG_PI_API_KEY}'`,
...
WRAPPER
	...
	if (startupFailed) {
		...
		console.log(chalk.white("API Key:     ") + chalk.yellow(process.env.DG_PI_API_KEY || "(not set)"));
		console.log(chalk.cyan("─".repeat(50)));

		console.log(`\n${chalk.bold("Export for shell:")}`);
		console.log(chalk.gray(`export OPENAI_BASE_URL="http://${host}:${port}/v1"`));
		console.log(chalk.gray(`export OPENAI_API_KEY="${process.env.DG_PI_API_KEY || "your-api-key"}"`));
